...
(async () => {
    ...
    await install('a4b05', 'ArbosActs'         , 'EcoOS'             , 'EVM Hypervisor (go 1.17)')
    await install('00064', 'ArbSys'            , 'EcoSys'            , 'L2 Precompile (go 1.17)')
    await install('00065', 'ArbInfo'           , 'EcoInfo'           , 'L2 Precompile (go 1.17)')
    await install('00066', 'ArbAddressTable'   , 'EcoAddressTable'   , 'L2 Precompile (go 1.17)')
    await install('00067', 'ArbBLS'            , 'EcoBLS'            , 'L2 Precompile (go 1.17)')
    await install('00068', 'ArbFunctionTable'  , 'EcoFunctionTable'  , 'L2 Precompile (go 1.17)')
    await install('00069', 'ArbosTest'         , 'EcoosTest'         , 'L2 Precompile (go 1.17)')
    await install('0006b', 'ArbOwnerPublic'    , 'EcoOwnerPublic'    , 'L2 Precompile (go 1.17)')
    await install('0006c', 'ArbGasInfo'        , 'EcoGasInfo'        , 'L2 Precompile (go 1.17)')
    await install('0006d', 'ArbAggregator'     , 'EcoAggregator'     , 'L2 Precompile (go 1.17)')
    await install('0006e', 'ArbRetryableTx'    , 'EcoRetryableTx'    , 'L2 Precompile (go 1.17)')
    await install('0006f', 'ArbStatistics'     , 'EcoStatistics'     , 'L2 Precompile (go 1.17)')
    await install('00070', 'ArbOwner'          , 'EcoOwner'          , 'L2 Precompile (go 1.17)')
    ...
    await install('000ff', 'ArbDebug'          , 'EcoDebug'          , 'L2 Precompile (go 1.17)')
